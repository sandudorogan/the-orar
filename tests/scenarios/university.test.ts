...
describe("Scenario: University faculty (10 instructors, 6 courses, 8 rooms)", () => {
	function build() {
		...
		const _courses = ["CS101", "CS201", "CS301", "MATH101", "MATH201", "PHY101"]
		...
		project.activities.push(
			createActivity({
				name: "CS101 Lecture",
				subjectName: "CS101",
				teacherIds: [instructors[0]!.id],
				classGroupIds: [groups[0]!.id, groups[1]!.id],
				totalPerWeek: 2,
				roomTags: ["lecture"],
			}),
			createActivity({
				name: "CS101 Lab Y1A",
				subjectName: "CS101",
				teacherIds: [instructors[1]!.id],
				classGroupIds: [groups[0]!.id],
				totalPerWeek: 1,
				roomTags: ["lab"],
			}),
			createActivity({
				...
				subjectName: "CS101",
				teacherIds: [instructors[1]!.id],
				classGroupIds: [groups[1]!.id],
				totalPerWeek: 1,
				roomTags: ["lab"],
			}),
			createActivity({
				name: "MATH101 Lecture",
				subjectName: "MATH101",
				teacherIds: [instructors[2]!.id],
				classGroupIds: [groups[0]!.id, groups[1]!.id],
				totalPerWeek: 3,
				roomTags: ["lecture"],
			}),
		...
		project.activities.push(
			createActivity({
				name: "CS201 Lecture",
				subjectName: "CS201",
				teacherIds: [instructors[3]!.id],
				classGroupIds: [groups[2]!.id, groups[3]!.id],
				totalPerWeek: 2,
				roomTags: ["lecture"],
			}),
			createActivity({
				name: "CS201 Lab Y2A",
				subjectName: "CS201",
				teacherIds: [instructors[4]!.id],
				classGroupIds: [groups[2]!.id],
				totalPerWeek: 2,
				roomTags: ["lab"],
			}),
			createActivity({
				...
				subjectName: "CS201",
				teacherIds: [instructors[4]!.id],
				classGroupIds: [groups[3]!.id],
				totalPerWeek: 2,
				roomTags: ["lab"],
			}),
			createActivity({
				name: "MATH201 Lecture",
				subjectName: "MATH201",
				teacherIds: [instructors[5]!.id],
				classGroupIds: [groups[2]!.id, groups[3]!.id],
				totalPerWeek: 2,
				roomTags: ["lecture"],
			}),
		...
		project.activities.push(
			createActivity({
				name: "CS301 Lecture",
				subjectName: "CS301",
				teacherIds: [instructors[6]!.id],
				classGroupIds: [groups[4]!.id, groups[5]!.id],
				totalPerWeek: 2,
				roomTags: ["lecture"],
			}),
			createActivity({
				name: "CS301 Lab Y3A",
				subjectName: "CS301",
				teacherIds: [instructors[7]!.id],
				classGroupIds: [groups[4]!.id],
				totalPerWeek: 2,
				roomTags: ["lab"],
			}),
			createActivity({
	...
	it("assigns rooms matching tags when possible", () => {
		...
		const lectureRoomIds = project.classrooms
